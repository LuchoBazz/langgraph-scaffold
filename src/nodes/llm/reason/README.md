...
This folder contains the implementation for the **reason** LLM node of the Lang Graph Scaffold agent.
...
The reason LLM node is responsible for:
...
```python
from src.nodes.llm.reason.processor import ReasonLLMNode

# Initialize the LLM node
node = ReasonLLMNode(config)
