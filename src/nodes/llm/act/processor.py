...
class ActLLMNode(BaseLLMNode):
    """
    Act LLM node implementation
    
    This node processes input using LLM capabilities for the act functionality.
    Customize the run() method to implement your specific LLM logic and AI system calls.
    """
    
    def __init__(self, config):
        super().__init__(config, "act")
        # Load node-specific prompts
        self.node_prompts = get_prompts()
    
    async def run(self, input_data: Any) -> Any:
        """
        Execute the act LLM logic with comprehensive error recovery.
        ...
        for attempt in range(max_retries):
            try:
                ...
                result = await self._execute_act_llm_logic(input_data)
    ...
    async def _execute_act_llm_logic(self, data: Any) -> Any:
        """
        Execute the specific act LLM logic
